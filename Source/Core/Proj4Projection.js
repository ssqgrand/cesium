...
function Proj4Projection(options) {
  ...
  const wkt = options.wellKnownText;
  const heightScale = defaultValue(options.heightScale, 1.0);
  const wgs84Bounds = defaultValue(options.wgs84Bounds, Rectangle.MAX_VALUE);
  const projectedBounds = defaultValue(options.projectedBounds, UNBOUNDED);
...
Proj4Projection.deserialize = function (serializedMapProjection) {
  ...
  const wgs84Bounds = Rectangle.unpack(json.wgs84Bounds);
  const projectedBounds = Rectangle.unpack(json.projectedBounds);
...
const scratchProjectionArray = [0.0, 0.0];
/**
...
Proj4Projection.prototype.project = function (cartographic, result) {
  ...
  const wgs84Bounds = this.wgs84Bounds;
  ...
  const projected = this._projection.forward(scratchProjectionArray);
...
Proj4Projection.prototype.unproject = function (cartesian, result) {
  ...
  const projectedBounds = this.projectedBounds;
  ...
  const unprojected = this._projection.inverse(scratchProjectionArray);
